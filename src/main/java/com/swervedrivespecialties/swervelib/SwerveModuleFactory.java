...
public class SwerveModuleFactory<DriveConfiguration, SteerConfiguration> {
  ...
  public SwerveModule create(ShuffleboardLayout container,
    ...
    var steerContainer = steerControllerFactory.create(
        container, steerConfiguration, moduleConfiguration);
  ...
  private static class ModuleImplementation implements SwerveModule {
    ...
    public void set(double driveVoltage, double steerAngle) {
      ...
      // If the difference is greater than 90 deg or less than -90 deg the drive
      // can be inverted so the total movement of the module is less than 90 deg
      if (difference > Math.PI / 2.0 || difference < -Math.PI / 2.0) {
        // Only need to add 180 deg here because the target angle will be put
        // back into the range [0, 2pi)
        steerAngle += Math.PI;
        driveVoltage *= -1.0;
      }
