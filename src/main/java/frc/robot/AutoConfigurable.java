...
public class AutoConfigurable {
    ...
    public void UpdateVariables() {
        ...
        for (Field classField : dontGrabFrom.allFields) {
            ...
            if (classField.getType() == double.class) {
                ...
                boolean newValue = (boolean) (dontGrabFrom.shuffleBoardElements.get(classField.getName())
                        .getBoolean(true));
    ...
    public List<Field> getAllVariablesAndInit(Class<?> getFieldsFrom, List<Field> fieldsSoFar) {
        if (fieldsSoFar == null) {
            fieldsSoFar = new ArrayList<>();
        }
        if (getFieldsFrom == null) {
            getFieldsFrom = this.getClass();
        }
    ...
    public void initShuffles(Field fieldToInit, Class<?> classFieldIn) {
        ...
        if (fieldToInit.getType() == int.class || fieldToInit.getType() == float.class
                || fieldToInit.getType() == double.class) {
            ...
            // System.out.println("The variable:" + fieldToInit.getName() + " is of type ("
            // + fieldToInit.getType() +") which the current auto config shuffle baord code
            // cannot process, curent avalible types are int, double, float , and
            // boolean.");
        }
    }
