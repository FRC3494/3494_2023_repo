...
public class TeleopDrive extends CommandBase {
    ...
    public TeleopDrive(Drivetrain drivetrain, Forearm forearm, Wrist wrist) {
         ...
         * });
         * 
         * OI.forearmFineAdjustPositiveEvent().rising().ifHigh(() ->
    ...
    public void execute() {
        ...
        if (forearmFineAdjust != 0)
            forearm.enableDirectDrive();
        else
            forearm.disableDirectDrive();

        forearm.directDrive(forearmFineAdjust);

        double wristFineAdjust = OI.wristFineAdjust();

        if (wristFineAdjust != 0)
            wrist.enableDirectDrive();
        else
            wrist.disableDirectDrive();

        wrist.directDrive(wristFineAdjust);
